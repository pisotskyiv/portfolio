...
2b. guidance: knowledge-graph refresh (optional local tooling, local-only — not published). If the `graphify` CLI is installed and `graphify-out/` exists, run `graphify update .` (AST-only re-extract, no LLM cost) so the graph matches the code being shipped. `public/graph.html` is gitignored and no longer committed or deployed — this step keeps the local graph current for querying, it does not produce anything that ships. Check: after `graphify update .`, `graphify query "what changed"` (or a diff of `graphify-out/manifest.json` against `git diff --name-only` for the range being shipped) shows no source file touched by this push that the graph missed. Skip silently when graphify is absent — the deploy must never depend on machine-local tooling. No executable gate check exists (graphify is not in CI); tracked in the missing-checks backlog (`CLAUDE.local.md`).

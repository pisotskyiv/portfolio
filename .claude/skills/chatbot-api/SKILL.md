...
3. Select the model through the failover wrapper, never an inline model id. Provider policy lives in `lib/chat-fallback.ts`:
   ...
   - Every `doStream` attempt runs under a first-token deadline (`FIRST_TOKEN_TIMEOUT_MS`, 30s): a provider that never resolves or opens a stream and emits nothing is aborted and thrown as `FirstTokenTimeoutError` (carries `elapsedMs`, logged in the route's failover line), which classifies transient and fails over like a 429 (Rule 7). `stream-start` parts are SDK bookkeeping, not a first token. This only guards a silent hang — a real reject (429/5xx) fails over immediately regardless of this value — and is kept with real margin under the route's 55s stream-abort/60s `maxDuration` so a failover still leaves the next provider time to answer.
